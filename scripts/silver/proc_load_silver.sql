...
AS $$
DECLARE
    -- Define tracking stopwatches
    start_time TIMESTAMP;
    end_time TIMESTAMP;
    batch_start_time TIMESTAMP;
    batch_end_time TIMESTAMP;
BEGIN
     -- Record whole-pipeline start time
    ...
    SELECT 
        ...
        -- Normalize marital status to user-friendly format
        CASE UPPER(TRIM(t.cst_marital_status ))
            WHEN 'S' THEN 'Single'
            WHEN 'M' THEN 'Married'
            ELSE 'n/a'
        END AS cst_marital_status,
            -- Normalize gender to user-friendly format
    ...
    ) t WHERE flag_last = 1; -- Keep only the most recent record per customer
    ...
    SELECT 
        prd_id,
        REPLACE(SUBSTRING(prd_key FROM 1 FOR 5), '-', '_') AS prd_cat_id, -- Extract category id
  ...
  -- Loading silver.erp_cust_az12
    ...
    SELECT 
        CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid FROM 4) -- Remove 'NAS' prefix if present
            ELSE cid
        END AS cid,
 ...
 -- Loading silver.erp_cat_g1v2
    ...
    RAISE NOTICE '>> Inserting Data Into: silver.erp_cat_g1v2';
...
EXCEPTION 
    WHEN OTHERS THEN
        ...
        RAISE NOTICE '========================================================';
        RAISE NOTICE '❌ CRITICAL PIPELINE EXCEPTION CAUGHT!';
        RAISE NOTICE '========================================================';
        RAISE NOTICE 'Error Message   : %', SQLERRM;
        RAISE NOTICE 'Error Location  : %', SQLSTATE;
        RAISE NOTICE '========================================================';
        
        -- Propagate the error upward so external orchestrators (like Python) know it failed
        RAISE EXCEPTION 'Silver processing aborted due to internal table transformation errors.';
END;
$$;
